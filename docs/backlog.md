...
**PWA:**
- **Push unsubscribe UI** — PushOptIn's subscribed state is a non-interactive `span` (`components/PushOptIn.jsx:95-100`); there's no in-app way to opt out (only iOS Settings). Before driving install/opt-in via an install prompt, make the subscribed bell tappable → `pushSubscription.unsubscribe()` + remove the `push_subscriptions` row, then flip the UI back to the enable state. NOTE: the server side already exists — `DELETE /api/push-subscription` (`app/api/push-subscription/route.js:45-74`) takes `{ endpoint }` and deletes only the caller's own row. So this is client-only work: `getSubscription()` → `unsubscribe()` → `DELETE` with that endpoint.

...
import { snapToCalendarQuarter } from '../financials/calendarQuarter.js';
...
function periodRowsQuarterly(symbol, incQ, bsQ, cfQ) {
  ...
  for (const inc of asArr(incQ)) {
    const date = inc?.date || null;
    // Calendar key via the shared snap (nearest boundary within tolerance) — the SAME
    // function valuation.js's calIndex uses, so storage and lookup can't diverge. Rows
    // without a parseable date get no key and are skipped.
    const cal = date ? snapToCalendarQuarter(date) : null;
    if (!cal) continue;
    rows.push({
      symbol,
      calendar_year: cal.year,
      calendar_quarter: cal.quarter,   // 1..4

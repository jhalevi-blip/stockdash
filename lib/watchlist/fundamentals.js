...
export function computeDerived(raw, screenRow) {
  ...
  // NOPAT = EBIT * (1 - tax rate). Invested capital is now computed from OUR balance
  // sheet (not FMP's investedCapitalTTM):
  //   invested_capital_reported    = total_debt + total_equity - cash
  //   invested_capital_ex_goodwill = reported - goodwill - intangibles  (primary)
  // `roic` divides by the ex-goodwill base (primary), `roic_reported` by reported.
  // goodwill/intangibles absent -> treated as 0 (no such line == none to subtract);
  // the debt/equity/cash base must be present or invested capital is NULL.
  ...
  const _gw = num(bs0.goodwill) ?? 0, _intang = num(bs0.intangibleAssets) ?? 0;
  const investedCapitalReported = (_debt !== null && _eq !== null && _cash !== null) ? _debt + _eq - _cash : null;
  const investedCapitalExGoodwill = investedCapitalReported === null ? null : investedCapitalReported - _gw - _intang;

...
function mapQuoteRow(row) {
  return {
    ...
    // FMP timestamp is epoch seconds; fall back to now so a row always has an
    // "as of" (spec §4 — a missing timestamp reads as "nothing happening").
    asOf:       row.timestamp ? row.timestamp * 1000 : Date.now(),
  };
}
...
export async function getQuotesForGroup(symbols, { fmpKey, label }) {
  ...
  const entries = await Promise.all(sorted.map(async sym => {
    ...
    if (!row) return [sym, { error: 'no quote returned' }];

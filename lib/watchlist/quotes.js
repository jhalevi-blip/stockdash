...
import { exchangeCurrency } from '../quoteCurrency.js';
...
function mapQuoteRow(row) {
  return {
    ...
    // Currency the quote is denominated in, derived from the exchange (FMP omits it).
    // null when the exchange is unknown → the dashboard treats that as unpriceable
    // for a non-USD holding rather than assuming USD.
    currency:   exchangeCurrency(row.exchange),

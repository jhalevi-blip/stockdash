...
/**
 * Client-safe holdings signature — sorted "TICKER:shares" of equities only.
 * Used to detect whether the held tickers/shares actually changed (e.g.
 * optimistic-cache vs network reconcile) so callers can skip redundant work.
 * NOT a crypto hash: do NOT confuse with computeHoldingsFingerprint in
 * holdingsFingerprint.js, which uses node:crypto and is server-only.
 */
export function holdingsSignature(holdings) {
  return (holdings ?? [])

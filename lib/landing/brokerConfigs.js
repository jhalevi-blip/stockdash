...
export const UI_STRINGS = {
  en: {
    demo: {
      ...
      // Privacy claim — must be exact per broker. DEGIRO resolves ISINs; Saxo carries
      // ticker symbols directly, so only those are sent. Default (homepage/DEGIRO) is
      // unchanged. `broker` is undefined on the homepage.
      trust: (broker) =>
        broker === 'saxo'
          ? 'Your file never leaves your browser — it’s parsed on your device. To price your holdings, only the ticker symbols are sent to a lookup service. No account, and your transaction file is never uploaded.'
      ...
      errors: {
        ...
        too_many_positions: {
          title: (d) => `The demo charts up to ${d.charted} positions`,
          body: (d) =>
            `The anonymous demo can chart up to ${d.charted} positions. This account has traded ${d.traded}, so we can’t show a return here without leaving parts of it out.`,
          cta: 'The full dashboard isn’t capped this way — create a free account to chart your whole portfolio.',
    ...
    result: {
      ...
      split: (arr) =>
        `We found a stock split in your file, and Saxo doesn’t tell us the ratio, so ${arr.length === 1 ? 'this position' : 'these positions'} can’t be computed and ${arr.length === 1 ? 'was' : 'were'} excluded: ${arr.slice(0, 8).join(', ')}${arr.length > 8 ? '…' : ''}.`,
  ...
  nl: {
    demo: {
      ...
      // Privacy claim — moet per broker exact zijn. DEGIRO gebruikt ISINs; Saxo draagt
      // tickersymbolen rechtstreeks, dus alleen die worden verstuurd.
      trust: (broker) =>
        broker === 'saxo'
          ? 'Je bestand verlaat je browser niet — het wordt op je apparaat verwerkt. Om je posities te prijzen, worden alleen de tickersymbolen naar een opzoekdienst gestuurd. Geen account, en je transactiebestand wordt nooit geüpload.'
      ...
      errors: {
        ...
        too_many_positions: {
          title: (d) => `De demo toont maximaal ${d.charted} posities`,
          body: (d) =>
            `De anonieme demo kan maximaal ${d.charted} posities tonen. Dit account heeft er ${d.traded} verhandeld, dus we kunnen hier geen rendement laten zien zonder delen ervan weg te laten.`,
          cta: 'Het volledige dashboard kent deze limiet niet — maak een gratis account aan om je hele portefeuille te tonen.',
          ctaButton: 'Maak een gratis account aan',
        },
        no_positions: {
    ...
    result: {
      ...
      split: (arr) =>
        `We vonden een aandelensplitsing in je bestand, en Saxo geeft ons de verhouding niet, dus ${arr.length === 1 ? 'deze positie kan' : 'deze posities kunnen'} niet worden berekend en ${arr.length === 1 ? 'is' : 'zijn'} uitgesloten: ${arr.slice(0, 8).join(', ')}${arr.length > 8 ? '…' : ''}.`,
...
// ── Saxo — English (the demo now parses Saxo transaction exports end to end) ──
// Same shape as degiroEn. English only for now; Dutch Saxo can follow. The copy is
// deliberately honest about the two demo limits — options aren't charted, and a
// position hit by a stock split is excluded (Saxo states no ratio) — so it doesn't
// promise a number we can't stand behind. `demoHeadline` overrides the shared
// dropzone headline so a Saxo visitor isn't told to drop a DEGIRO statement.
export const saxoEn = {
  slug: 'saxo',
  path: '/saxo',
  name: 'Saxo',
  lang: 'en',
  hreflang: { 'en': '/saxo', 'nl': '/nl/saxo', 'x-default': '/saxo' },
  demoHeadline: 'Drop your Saxo transaction export here',
  // The shared "how it works" walkthrough is DEGIRO-illustrated (real DEGIRO
  // screenshots + "Export from DEGIRO" copy). Hide it on Saxo until there's a Saxo
  // walkthrough — showing DEGIRO screenshots to a Saxo visitor is the wrong broker.
  // A future Saxo walkthrough is: drop this flag + broker-aware DTStepsWalkthrough.
  hideWalkthrough: true,
  meta: {
    title: 'Saxo Portfolio Tracker — See Your Real Return, Free | StockDashes',
    description:
      'Track your Saxo portfolio and see your real, time-weighted return — historical FX handled, benchmarked against the market. Drop your Saxo transaction export; it never leaves your browser. Free, no account.',
  },
  copy: {
    eyebrow: 'For Saxo investors',
    h1: 'The free Saxo portfolio tracker that shows your real return',
    subhead:
      'Drop your Saxo transaction export and see your true time-weighted return in euros — currency handled, benchmarked against the market. No broker login, nothing uploaded. Options aren’t charted, and a position hit by a stock split is left out — Saxo doesn’t give us the ratio, so we won’t guess.',
    ctaPrimary: 'See my real return',
    ctaSecondary: 'Sign up free',
    trustFine:
      'No broker login. Read-only export. Your transaction file is parsed in your browser and never leaves your device — only the ticker symbols are sent to look up prices.',
    problemTitle: 'What the Saxo demo does — and what it leaves out',
    problemLead:
      'Drop your Saxo transaction export (the Transacties file) and we rebuild your time-weighted return in euros, with historical FX separated from stock performance and a market benchmark alongside. Two honest limits, both shown on screen: option legs are counted and set aside — we don’t chart options — and any position affected by a stock split is excluded, because Saxo marks the split without its ratio and we won’t show a number we’d have to guess at.',
    // No problem-section links: Saxo has no equivalent blog post, and pointing Saxo
    // visitors at DEGIRO content is the same broker leak we're removing elsewhere.
    problemLinks: [],
    faqTitle: 'Saxo portfolio tracker — FAQ',
    finalTitle: 'See your real Saxo return',
    finalCta: 'Get started — it’s free',
  },
  faq: [
    {
      q: 'Which Saxo file do I need?',
      a: 'Your Saxo transaction export — the Transacties spreadsheet, as XLSX or CSV. The demo reads your trades from it directly; there’s no broker login.',
    },
    {
      q: 'Does it handle options?',
      a: 'No. Option legs in your export are counted and then excluded from the return — the demo charts stock positions only, so options are never folded into the number. You’ll see how many were set aside.',
    },
    {
      q: 'What about stock splits?',
      a: 'Saxo books a split as a marker without the ratio, and lists your pre-split buy against the post-split sell. Rather than guess the ratio and show a wrong return, the demo excludes any position affected by a split and names it on screen.',
    },
    {
      q: 'Is my data safe?',
      a: 'Your transaction file is parsed entirely in your browser and never uploaded. Only the ticker symbols are sent to a lookup service to fetch prices. There is no broker login and nothing to delete.',
    },
    {
      q: 'What is a time-weighted return?',
      a: 'A return that splits your history at every deposit and withdrawal and links the pieces, so the timing of your own contributions can’t flatter or punish the result. It measures how your decisions performed — the number worth comparing to an index.',
...
// ── Saxo — Dutch (written native, not translated; targets the "saxo rendement"
//    cluster, uncovered until now). Same honest limits as saxoEn: the demo reads
//    the Saxo transaction export, options aren't charted, and a position hit by a
//    stock split is excluded (Saxo states no ratio). hideWalkthrough carries over —
//    the shared walkthrough is still DEGIRO-illustrated. No problem-section link:
//    there's no Dutch Saxo blog post, and pointing Dutch Saxo visitors at English
//    DEGIRO content is the broker/language leak we removed elsewhere. ─────────────
export const saxoNl = {
  slug: 'saxo',
  path: '/nl/saxo',
  name: 'Saxo',
  lang: 'nl',
  hreflang: { 'en': '/saxo', 'nl': '/nl/saxo', 'x-default': '/saxo' },
  demoHeadline: 'Sleep hier je Saxo-transactie-export',
  hideWalkthrough: true,
  meta: {
    title: 'Saxo rendement berekenen — je werkelijke rendement, gratis | StockDashes',
    description:
      'Bereken je werkelijke Saxo-rendement: tijdgewogen, met valuta-effect apart en afgezet tegen een benchmark. Sleep je Saxo-transactie-export erin en zie in seconden je echte rendement. Gratis, geen account, niets wordt geüpload.',
  },
  copy: {
    eyebrow: 'Voor Saxo-beleggers',
    h1: 'Bereken je werkelijke Saxo-rendement — gratis',
    subhead:
      'Sleep je Saxo-transactie-export erin en zie je echte tijdgewogen rendement in euro’s — valuta apart gerekend, afgezet tegen de markt. Geen brokerlogin, niets wordt geüpload. Opties worden niet in beeld gebracht, en een positie die door een aandelensplitsing is geraakt laten we weg — Saxo geeft ons de verhouding niet, dus we gokken niet.',
    ctaPrimary: 'Bekijk mijn rendement',
    ctaSecondary: 'Gratis aanmelden',
    trustFine:
      'Geen brokerlogin. Alleen-lezen export. Je transactiebestand wordt in je browser verwerkt en verlaat je apparaat nooit — alleen de tickersymbolen worden verstuurd om koersen op te zoeken.',
    problemTitle: 'Wat de Saxo-demo doet — en wat niet',
    problemLead:
      'Sleep je Saxo-transactie-export (het Transacties-bestand) erin en we bouwen je tijdgewogen rendement in euro’s opnieuw op, met historische valuta losgetrokken van je aandelenprestatie en een benchmark ernaast. Twee eerlijke beperkingen, allebei op het scherm getoond: optieregels worden geteld en apart gezet — we brengen opties niet in beeld — en elke positie die door een aandelensplitsing is geraakt wordt uitgesloten, omdat Saxo de splitsing zonder verhouding vastlegt en we geen getal tonen dat we zouden moeten gokken.',
    // Geen problem-section links: er is nog geen Nederlandstalige Saxo-blogpost, en
    // Nederlandse Saxo-bezoekers naar Engelse DEGIRO-content sturen is de leak die we
    // eerder hebben weggehaald.
    problemLinks: [],
    faqTitle: 'Saxo rendement berekenen — veelgestelde vragen',
    finalTitle: 'Zie je werkelijke Saxo-rendement',
    finalCta: 'Begin gratis',
  },
  faq: [
    {
      q: 'Welk Saxo-bestand heb ik nodig?',
      a: 'Je Saxo-transactie-export — het Transacties-overzicht, als XLSX of CSV. De demo leest je transacties er rechtstreeks uit; er is geen brokerlogin.',
    },
    {
      q: 'Werkt het met opties?',
      a: 'Nee. Optieregels in je export worden geteld en daarna uitgesloten van het rendement — de demo brengt alleen aandelenposities in beeld, dus opties worden nooit in het getal verwerkt. Je ziet hoeveel er apart zijn gezet.',
    },
    {
      q: 'En aandelensplitsingen?',
      a: 'Saxo legt een splitsing vast als markering zonder de verhouding, en zet je aankoop van vóór de splitsing tegenover de verkoop erna. In plaats van de verhouding te gokken en een onjuist rendement te tonen, sluit de demo elke positie uit die door een splitsing is geraakt en benoemt die op het scherm.',
    },
    {
      q: 'Zijn mijn gegevens veilig?',
      a: 'Je transactiebestand wordt volledig in je browser verwerkt en wordt nooit geüpload. Alleen de tickersymbolen worden naar een opzoekdienst gestuurd om koersen op te halen. Er is geen brokerlogin en er valt niets te verwijderen.',
    },
    {
      q: 'Wat is tijdgewogen rendement?',
      a: 'Een rendement dat je historie opknipt bij elke storting en opname en die stukken aan elkaar koppelt, zodat de timing van je eigen inleg het resultaat niet kan vertekenen. Het meet hoe je keuzes presteerden — het getal dat je met een index kunt vergelijken.',

...
export async function parseFileInBrowser(file) {
  ...
  // OpenFIGI names came through parseDeGiro (no second resolution pass); FMP
  // names come from one company-profile round-trip.
  const figiNameByTicker = parsed.namesByTicker ?? {};
  const profiles = await fetchProfiles(coveredTickers);
  const identityOk = new Set();
  for (const t of coveredTickers) {
    const figiName = figiNameByTicker[t];

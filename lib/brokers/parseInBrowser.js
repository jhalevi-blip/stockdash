...
export async function parseFileInBrowser(file) {
  ...
  // Unsized OPEN positions are their own bucket — neither matched nor identity-
  // excluded — so keep them out of the total/excluded accounting.
  const unsizedOpenCount = openTickers.filter((t) => unsizedSet.has(t)).length;
  const finalTotal    = (openTickers.length - unsizedOpenCount) + unresolvedCount;

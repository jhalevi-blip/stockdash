...
// Calendar index (year*4 + quarter-1) from an ISO period-end date, for finding the row
// exactly three years (12 quarters) earlier by key. null on an unparseable date.
 ...
 */
export function buildValuationRows(ttmRows = [], priceAt = () => null) {
  const byCal = new Map();
  for (const r of ttmRows) { const c = calIndex(r.date); if (c !== null) byCal.set(c, r); }

  return ttmRows.map(r => {
    const eps = ttmEps(r);
    const price = num(priceAt(r.date));

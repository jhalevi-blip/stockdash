...
import { calIndex } from './calendarQuarter.js';
...
function shapeRow({ date, label, revenue, grossProfit, operatingIncome, netIncome, nd, ttmEbitda, sharesDiluted }) {
  ...
  return {
    ...
    // Diluted share count at the window's latest quarter (point-in-time, never summed),
    // for TTM EPS in the valuation chart. null unless supplied — only the ttm builder
    // passes it; quarterly/annual rows carry null and existing consumers ignore it.
    sharesDiluted: num(sharesDiluted),
  };
}

// Calendar index of a quarterly row (from its STORED calendar key, via the shared
// helper), used to detect 4 CONSECUTIVE quarters so a rolling sum never spans a gap
// (a missing quarter would otherwise understate TTM).
const qIndex = r => calIndex(r.calendar_year, r.calendar_quarter);
  ...
  for (let i = 0; i < qs.length; i++) {
    ...
    if (sums) {
      const tRow = shapeRow({
        ...
        sharesDiluted: r.weighted_average_shares_diluted,   // latest quarter of the window
      });

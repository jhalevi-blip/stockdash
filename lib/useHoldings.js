...
import { getCachedHoldings, saveUserHoldings, holdingsSignature } from '@/lib/holdingsStorage';
...
export function useHoldings() {
  ...
  const [settings,   setSettings]   = useState(null); // portfolios.settings | null (loading)
  ...
  useEffect(() => {
    ...
    if (isSignedIn && user?.id) {
      // Optimistic hydration: seed from the owner-checked write-through cache so
      // holdings-keyed fetches (prices/history/sectors) can start before the
      // /api/portfolio round-trip resolves. getCachedHoldings enforces
      // owner === user.id (the July 2 H2 guard) → never exposes another user's
      // cache. Falls back to null (loading) when the cache is empty/foreign.
      const cached = getCachedHoldings(user.id);
      setHoldings(cached.length ? cached : null);
      setError(null);
      ...
      fetch('/api/portfolio')
        ...
        .then((data) => {
          ...
          if (data.signedIn && data.holdings?.length) {
            // Preserve the optimistic reference when tickers+shares are unchanged,
            // so the holdings-keyed effects don't fire a second (redundant) fetch.
            setHoldings(prev =>
              holdingsSignature(prev) === holdingsSignature(data.holdings) ? prev : data.holdings
            );
          ...
          setSettings(data.settings ?? {}); // portfolios.settings (Supabase-authoritative)
        })
        .catch((err) => {
          if (cancelled) return;
      ...
      // The foreign-cache wipe now lives in GuestDataGuard (app-wide); the
      // getCachedHoldings getter still guards what an anonymous viewer sees.
      setHoldings(getLocalHoldings());
      setCash(null);
      setSettings({}); // no server settings for anonymous viewers
      setError(null);
    }
  ...
  return {
    ...
    settings, // portfolios.settings object | null (null while loading) — Supabase-authoritative
    error,

...
export default function FinancialFilingsV2Page() {
  ...
  /* ─── Fetch filings + news in parallel when a ticker is selected ──────── */
  ...
  /* ─── Lazy-load transcripts only when that tab is first opened ───────── */
  const loadTranscripts = async (ticker) => {
    if (transcripts.length) return; // already loaded for current ticker
    setTxLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (autoTicker) loadData(autoTicker);
    } catch {
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 2 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '6px 0 0', maxWidth: 600 }}>
          SEC filings, company news, and earnings call transcripts for your portfolio holdings
        </p>
      </div>
      ...
      {tickers.length > 0 && !selected && (
        <div className="chart-placeholder">Select a stock to view SEC filings and news</div>
      )}
      {loading && (
        <div className="chart-placeholder">Loading research data for {selected}…</div>
      )}

      {/* ── Tabbed data view ─────────────────────────────────────────────── */}
      {!loading && selected && (
        <>
          {/* Tab bar — flexWrap so 3 tabs don't overflow at 375px */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {[
              ['filings',     '📄 SEC Filings'],
              ['news',        '📰 Latest News'],
              ['transcripts', '🎙️ Earnings Calls'],
            ].map(([key, label]) => (
              <button
                key={key}
                onClick={() => {
                  setTab(key);
                  if (key === 'transcripts') loadTranscripts(selected);
                }}
                style={{
                  ...btnBase,
                  padding: '7px 16px',
          ...
          {/* ── Earnings Calls tab (lazy-loaded transcripts) ─────────────── */}
          {tab === 'transcripts' && (
            <>
              {txLoading ? (
      ...
      {/* ── Earnings Call Transcripts — external links ────────────────────── */}
      {/* Renders for ALL portfolio tickers (or demo tickers when logged out).  */}
      {/* Visible regardless of which ticker is selected in the picker above.  */}
      {tickers.length > 0 && (
        <div style={{ marginTop: 26, borderTop: '1px solid var(--border-color)', paddingTop: 24 }}>
          <div style={{
            fontSize: 11, fontWeight: 700, letterSpacing: '.08em',
            textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4,
          }}>
